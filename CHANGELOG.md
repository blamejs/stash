...
The default `'restore'` policy has always kept a failed or crashed read's
entry for retry. `'burn'` is the opt-in that destroys a read entry on the
assumption that a read attempt may have observed its bytes -- but crash
recovery applied it even to a claim whose process died before streaming a
single byte, turning a crash before any read into silent data destruction
that burn's own rationale does not justify. Recovery is now delivery-gated:
it burns an orphaned `'burn'` claim only if a byte actually reached a
consumer, and restores one that never delivered. A live `'burn'` read that
fails mid-drain is still burned, because it demonstrably delivered bytes.
Backends learn a small, persistent delivery signal to make this decision
survive the crash, the conformance harness certifies it, and the store's
wall-clock posture -- and its behavior under a clock step in either direction
-- is now written down.
...
- Under `onPopFailure: 'burn'`, crash recovery destroyed a claim even when
  the crashed process had streamed no byte to any consumer, turning a crash
  before any read into silent data destruction. Recovery now burns an
  orphaned burn claim only if a byte actually reached a consumer; one that
  never delivered is restored, not destroyed (the default `'restore'` policy
  was always safe). A live burn read that fails mid-drain is still burned --
  it demonstrably delivered bytes -- so the opt-in guarantee is unchanged for
