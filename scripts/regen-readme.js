...
function harvestErrors() {
  ...
  if (rows.length === 0)
    throw new Error("regen-readme: no error classes harvested from src/errors.js");
  ...
  if (rows.length !== declared) {
    throw new Error(
      "regen-readme: harvested " +
        rows.length +
        " error rows but src/errors.js declares " +
        declared +
...
if (check) {
  if (next !== readme) {
    console.error(
      "[regen-readme] README.md error-code table is stale -- run `node scripts/regen-readme.js` and commit",
    );
    process.exit(1);
  }
  ...
  console.log(
    "[regen-readme] wrote README.md error-code table (" + harvestErrors().length + " codes)",
  );
}
