...
// reviewerReviewedHead(prNum, headSha) -- true once the reviewer bot has
// reviewed the PR's CURRENT head. The bot signals a review in two forms and
// both must count, or the wait times out on the common (clean) case:
//   (1) a formal review node whose reviewed commit is the head -- the form it
//       posts WITH findings; and
//   (2) a clean-verdict issue comment citing the head's commit sha, posted
//       with NO review node when it finds nothing.
function reviewerReviewedHead(prNum, headSha) {
  ...
  if (Array.isArray(reviews) && reviews.some((r) =>
    r && r.author && isReviewBot(r.author.login) && r.commit && r.commit.oid === headSha)) {
    return true;
  }
  const comments = ghJson(["pr", "view", String(prNum), "--json", "comments", "--jq", ".comments"]);
  const headPrefix = headSha.slice(0, 10);
  return Array.isArray(comments) && comments.some((c) =>
    c && c.author && isReviewBot(c.author.login) &&
