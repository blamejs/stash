...
for (const { name, create } of BACKENDS) {
  ...
  suite("conformance: " + name, () => {
    test("list accepts includeExpired and rejects unknown options", async () => {
      ...
      assert.deepEqual(
        (await stash.list({ includeExpired: true })).map((e) => e.id),
        [ref],
      );
      assert.deepEqual(
        (await stash.list({ includeExpired: false })).map((e) => e.id),
        [ref],
      );
    ...
    test("ttl attaches regardless of source type (Buffer and Readable)", async () => {
      ...
      const fromStream = await stash.show(
        await stash.push(Readable.from([Buffer.from("stream")]), { ttl: "1h" }),
      );
    ...
    test("the ttl override matrix: absent inherits, explicit overrides, null clears", async () => {
      ...
      assert.equal(
        (await withDefault.show(await withDefault.push("x", { ttl: null }))).expiresAt,
        null,
      );
    });
    ...
    test("list rejects a non-boolean includeExpired instead of leaking expired entries", async () => {
      ...
      for (const bad of ["false", "true", 1, 0, {}, []]) {
        await assert.rejects(
          stash.list({ includeExpired: bad }),
          (err) => err instanceof TypeError && !(err instanceof StashError),
        );
      }
    ...
    test("apply opens no read stream on an expired entry: zero reads, one remove", async () => {
      ...
      assert.equal(
        calls.filter((c) => c === "remove").length,
        1,
        "dropped in passing exactly once",
      );
    });
    ...
    test("apply re-checks expiry after opening the read: an entry lapsing in the open window is dropped, not served", async () => {
      ...
      assert.equal(
        readCalled,
        true,
        "the read WAS opened -- the reject came from the post-open re-check, not the pre-gate",
      );
    ...
    test(
      "apply's expiry re-check does not hang on a non-closing source (emitClose:false)",
      { timeout: 8000 },
      async () => {
    ...
    test("maxSize is enforced mid-stream: no chunk after the crossing one is pulled", async () => {
      ...
      async function* chunks() {
        ...
        } // 4,8,12 crosses at the third
      }
      await assert.rejects(stash.push(chunks()), SizeExceeded);
      assert.ok(
        pulled.length <= 3,
        "stopped at the crossing chunk, not drained (pulled " + pulled.length + ")",
      );
    });
    ...
    test("maxSize bounds a SharedArrayBuffer chunk the same as an ArrayBuffer", async () => {
      ...
      const stash = new Stash({ backend: create(), maxSize: 16 });
      async function* one() {
        yield new SharedArrayBuffer(64);
      } // 4x maxSize
      await assert.rejects(
        stash.push(one()),
        (e) => e instanceof SizeExceeded && e.code === "E2BIG",
    ...
    test("an unbounded source is abandoned at the boundary, not drained", async () => {
      ...
      assert.ok(
        yielded <= 9,
        "pulls bounded by ~ceil(maxSize/chunk), not infinite (" + yielded + ")",
      );
    });
    ...
    test("maxTotal: a push whose footprint exceeds the remaining headroom is StashFull; the partial is cleaned; a fitting re-push works", async () => {
      ...
      await assert.rejects(
        stash.push(Buffer.alloc(oneEntry, 2)),
    ...
    test("expired-but-unswept entries do not hold the door shut (maxEntries and maxTotal)", async () => {
      ...
      assert.deepEqual(
        (await s1.list({ includeExpired: true })).map((e) => e.id),
        [live1],
      );
    ...
    test("StashFull/SizeExceeded are StashErrors; a bad limit value is a config TypeError, not a StashError", async () => {
      ...
      assert.throws(
        () => new Stash({ backend: create(), maxSize: "nope" }),
        (e) => e instanceof TypeError && !(e instanceof StashError),
      );
    });
    ...
    test("an abandoned budgeted read spends nothing: readsLeft is unchanged, monotone", async () => {
      ...
      assert.equal(
        ...
        1,
        "only the completed read debited -- the abandon cost nothing",
      );
    });
    ...
    test("a failed budgeted read (digest mismatch) errors IntegrityError and spends nothing", async () => {
      ...
      assert.equal(
        (await stash.show(ref)).readsLeft,
        2,
        "a failed drain is not a spend -- drain AND digest match is",
      );
    });
    ...
    test("an unbudgeted apply stays lock-free: it takes no claim", async () => {
      ...
      const backend = wrapBackend(inner, {
        claim: (...a) => {
          seen.push("claim");
          return inner.claim(...a);
        },
        restore: (...a) => {
          seen.push("restore");
          return inner.restore(...a);
        },
        commit: (...a) => {
          seen.push("commit");
        ...
        consumeRead: (...a) => {
          seen.push("consumeRead");
          return inner.consumeRead(...a);
        },
      });
      const stash = new Stash({ backend });
    ...
    test("a drop during a live claim is monotone: the entry never serves again, and the abandon spends nothing", async () => {
      ...
      await assert.rejects(
        stash.show(ref),
        RefNotFound,
        "a dropped entry is gone, even while claimed",
      );
    ...
    test("verify().scanned counts a claimed entry, matching stats().entries on every backend (parity)", async () => {
      ...
      try {
        ...
        assert.equal(
          v.scanned,
          s.entries,
          "verify scans every entry stats counts -- claimed included",
        );
    ...
    test("clear() destroys EVERY entry before emitting -- a throwing listener cannot leave the store half-cleared", async () => {
      ...
      stash.on("dropped", () => {
        throw new Error("listener boom");
      }); // throws on the first emit
      await assert.rejects(stash.clear(), /listener boom/);
      assert.deepEqual(
        await stash.list(),
        [],
        "the whole store is cleared despite the throwing listener -- removes finish before events",
      );
    });
    ...
    test("prune() reaps EVERY expired entry before emitting -- a throwing listener cannot strand one", async () => {
      const stash = new Stash({ backend: create() });
      await stash.push("x", { ttl: 0 });
      await stash.push("y", { ttl: 0 });
      stash.on("expired", () => {
        throw new Error("listener boom");
      });
      await assert.rejects(stash.prune(), /listener boom/);
      assert.deepEqual(
        await stash.list({ includeExpired: true }),
        [],
        "all expired entries reaped despite the throwing listener",
      );
    });
    ...
    test(
      "a burned pop emits 'popped' -- a burn's delete lands, never a silent destruction (SPEC.md 4.3)",
      { timeout: 8000 },
      async () => {
        const stash = new Stash({ backend: create(), onPopFailure: "burn" });
    ...
    test(
      "an ASYNC-rejecting 'sweepError' listener is contained: no unhandledRejection, the janitor keeps sweeping",
      { timeout: 8000 },
      async () => {
        ...
        try {
          ...
          assert.deepEqual(
            unhandled,
            [],
            "an async-rejecting sweepError handler never becomes an unhandledRejection",
          );
        } finally {
    ...
    test(
      "a THROWING 'sweepError' listener cannot brick the janitor: the in-flight guard clears and later ticks still sweep",
      { timeout: 8000 },
      async () => {
        ...
        assert.deepEqual(
          fatal,
          [],
          "a throwing sweepError handler never escalates to the fatal 'error'",
        );
        await stash.close();
      },
    );
    ...
    test("for await (const entry of stash) yields the same id set as list()", async () => {
      ...
      const a = await stash.push("a");
      const b = await stash.push("b");
      const ids = [];
    ...
    test("store() files a fresh entry verbatim (step 6): identity preserved, show() returns the caller's fields", async () => {
      const stash = new Stash({ backend: create() });
      const id = generate();
      const entry = makeStoredEntry(id, "replicated bytes", {
        createdAt: 500,
        expiresAt: null,
        reads: 3,
        readsLeft: 2,
      ...
      assert.equal(
        await stash.store(entry, "replicated bytes"),
        true,
        "a fresh store returns true",
      );
      ...
      assert.deepEqual(
        await drain(await stash.apply(id)),
        Buffer.from("replicated bytes"),
    ...
    test("store() refuses a tombstoned id -> false, writes nothing (step 2)", async () => {
      ...
      assert.equal(
        await stash.store(entry, "resurrect me"),
        false,
        "a tombstoned id never comes back",
      );
    ...
    test("store() throws IntegrityError on a digest conflict (step 5): the original is untouched", async () => {
      ...
      await assert.rejects(
        stash.store(conflicting, "different"),
        (e) => e instanceof IntegrityError && e.code === "EINTEGRITY",
      );
      assert.deepEqual(
        await drain(await stash.apply(id)),
        Buffer.from("original"),
        "the existing entry still applies to the original bytes",
      );
    });
    ...
    test("store() reconciles identical bytes under a DIFFERENT algorithm as an idempotent no-op (step 4 by byte identity)", async () => {
      ...
      const id = generate();
      assert.equal(
        await stash.store(storedEntryUnder(id, bytes, "sha256"), bytes),
        true,
        "first store lands under sha256",
      );
      const sha3 = storedEntryUnder(id, bytes, "sha3-512");
      assert.equal(
        await stash.store(sha3, bytes),
        false,
        "identical bytes, different algorithm -> idempotent no-op",
      );
      assert.equal(
        await stash.store(sha3, bytes),
        false,
        "and the retry is still free -- reconciliation never wedges",
      );
      assert.equal((await stash.list()).length, 1, "exactly one entry -- no duplicate");
      assert.equal(
        (await stash.show(id)).digest,
        storedEntryUnder(id, bytes, "sha256").digest,
        "the stored entry keeps its ORIGINAL algorithm -- nothing was rewritten",
      );
      assert.deepEqual(
        await drain(await stash.apply(id)),
        Buffer.from(bytes),
      ...
      assert.equal(
        await stash.store(storedEntryUnder(id2, bytes, "sha256"), bytes),
        false,
        "existing sha3-512, incoming sha256 -> idempotent",
      );
    });
    ...
    test("store() no-ops a same-bytes cross-algorithm duplicate even when maxSize now sits below the entry (a lowered limit / tighter replica must not wedge anti-entropy)", async () => {
      ...
      assert.equal(
        await seeder.store(storedEntryUnder(id, bytes, "sha256"), bytes),
        true,
        "the entry is already present under sha256",
      );
      const tight = new Stash({ backend, maxSize: 10 }); // bytes.length >> 10
      assert.equal(
        await tight.store(storedEntryUnder(id, bytes, "sha3-512"), bytes),
        false,
        "identical bytes, different algorithm -> idempotent no-op, NOT SizeExceeded",
      );
      assert.equal((await tight.list()).length, 1, "still exactly one entry -- nothing rewritten");
      assert.equal(
        (await tight.show(id)).digest,
        storedEntryUnder(id, bytes, "sha256").digest,
        "the stored entry keeps its original algorithm",
      );
      ...
      await assert.rejects(
        tight.store(storedEntryUnder(id, bytes + "X", "sha3-512"), bytes + "X"),
        IntegrityError,
      );
      ...
      await assert.rejects(
        tight.store(storedEntryUnder(id, bytes, "sha3-512"), bytes + "EXTRA"),
        IntegrityError,
      );
    });
    ...
    test("store() still flags a CROSS-algorithm digest conflict as IntegrityError (step 5): different bytes never masquerade as idempotent", async () => {
      ...
      const id = generate();
      await stash.store(
        storedEntryUnder(id, "the original content", "sha256"),
        "the original content",
      );
      const conflicting = storedEntryUnder(id, "a genuinely different payload", "sha3-512"); // different bytes AND algorithm
      await assert.rejects(
        stash.store(conflicting, "a genuinely different payload"),
        (e) => e instanceof IntegrityError && e.code === "EINTEGRITY",
      );
      assert.deepEqual(
        await drain(await stash.apply(id)),
        Buffer.from("the original content"),
        "the existing entry is untouched",
      );
      assert.equal(
        (await stash.show(id)).digest,
        storedEntryUnder(id, "the original content", "sha256").digest,
        "and keeps its original algorithm/digest",
      );
    });
    ...
    test("store() catches a lying CROSS-algorithm entry (digest disagrees with its own bytes) -> IntegrityError, nothing changes", async () => {
      ...
      await assert.rejects(
        stash.store(lie, "STORED CONTENT"), // ...but streams different (same-length) bytes
        (e) => e instanceof IntegrityError && e.code === "EINTEGRITY",
      );
      assert.deepEqual(
        await drain(await stash.apply(id)),
    ...
    test("store() order is normative: step 2 (tombstoned) beats step 5 (digest conflict) -> false, not IntegrityError", async () => {
      const stash = new Stash({ backend: create() });
      const id = await stash.push("held");
      await stash.drop(id); // grave for id
      const conflicting = makeStoredEntry(id, "conflicting bytes");
      assert.equal(
        await stash.store(conflicting, "conflicting bytes"),
        false,
        "the grave wins over the conflict verdict",
      );
    });
    ...
    test("store() malformed id is InvalidRef before any backend access (step 1)", async () => {
      ...
      for (const bad of ["../../etc/passwd", "v1_..", "not-a-ref", ""]) {
        ...
        await assert.rejects(
          stash.store(entry, "x"),
          (e) => e instanceof InvalidRef && e.code === "EBADREF",
        );
      }
    });
    ...
    test("store() catches transfer corruption: a digest lie and a size lie both IntegrityError, nothing lands", async () => {
      ...
      const idB = generate();
      const sizeLie = makeStoredEntry(idB, "exact");
      sizeLie.size += 1; // digest right, size off by one
    ...
    test("store() is SILENT -- zero emissions across the whole event set (echo-suppression, SPEC.md 4.4)", async () => {
      const stash = new Stash({ backend: create() });
      const seen = [];
      for (const ev of ["pushed", "popped", "dropped", "expired"])
        stash.on(ev, () => seen.push(ev));
    ...
    test("store() rejects a meta that serializes to a scalar (Date / toJSON-scalar) -- no unreadable entry lands", async () => {
      ...
      for (const badMeta of [new Date(), { toJSON: () => 5 }, { toJSON: () => "scalar" }]) {
        ...
        const id = generate();
        await assert.rejects(
          stash.store(makeStoredEntry(id, "bytes", { meta: badMeta }), "bytes"),
          IntegrityError,
        );
        ...
        assert.deepEqual(
          await stash.list(),
          [],
          "the store holds no unreadable entry -- list() does not choke",
        );
      }
    });
    ...
    test("a Uint8Array from another realm is a Uint8Array: accepted as a source, not just as a chunk", async () => {
      ...
      assert.equal(
        await stash.store(entry, runInNewContext("new Uint8Array(n)", { n: [...bytes] })),
        true,
      );
    ...
    test("store() honors the stash limits like push: over maxSize -> SizeExceeded, past maxEntries/maxTotal -> StashFull, nothing lands", async () => {
      // A replicated entry is untrusted -- it must not slip the configured capacity.
      {
        ...
        const id = generate();
        await assert.rejects(
          stash.store(makeStoredEntry(id, "waaaay too many bytes"), "waaaay too many bytes"),
          SizeExceeded,
        );
    ...
    test("concurrent same-id store()s do not both land: conflicting replicas reconcile, no raw error, ONE consistent entry", async () => {
      ...
      // internally consistent -- never a torn mix of two replicas.
      {
        ...
        const stash = new Stash({ backend: create() });
        const id = generate();
        const racers = ["alpha", "bravo", "charlie", "delta"].map((b) =>
          stash.store(makeStoredEntry(id, b), b),
        );
        const settled = await Promise.allSettled(racers);
        for (const r of settled) {
          if (r.status === "rejected") {
            assert.ok(
              r.reason instanceof IntegrityError,
              "a losing conflict is IntegrityError, never a raw fs error: " +
        ...
        assert.equal(
          ...
          stored.digest,
          "the landed entry is not a torn mix of the racers",
        );
      }
      {
        ...
        for (const r of settled)
          assert.equal(r.status, "fulfilled", "identical concurrent stores settle cleanly");
        assert.equal(settled.filter((r) => r.value === true).length, 1, "one lands");
        assert.ok(
          settled.filter((r) => r.value === false).length >= 1,
          "the rest are idempotent no-ops",
        );
    ...
    test("reconcilable() is the resilient source read: healthy entries plus an empty corrupt set, a clean drop-in for the sync loop", async () => {
      ...
      assert.deepEqual(
        ...
        [a, budgeted, c].sort(),
        "the expired entry is filtered, the live ones listed",
      );
      ...
      assert.equal(
        (await to.show(budgeted)).readsLeft,
        2,
        "the budget rode through reconcilable() -> store() intact",
      );
    });
    ...
    test("a grave propagates through an EMPTY replica: a node that never held the id adopts the grave, refusing a later resurrection", async () => {
      ...
      const id = await A.push(payload);
      assert.equal(
        await C.store(makeStoredEntry(id, payload), payload),
        true,
        "C holds a live copy",
      );
      await drain(await A.pop(id)); // A pops it -> a grave on A
      // Sync A -> B: propagate A's grave to the EMPTY B (the done-when's drop loop).
      for (const g of await A.tombstones())
        assert.equal(await B.drop(g.id), false, "nothing live on B, but the grave is adopted");
      assert.equal(
        (await B.tombstones()).length,
        1,
        "B adopted the grave though it never held the id",
      );
      // A stale C now tries to re-file its live copy onto B -- the propagated grave refuses it.
      assert.equal(
        await B.store(makeStoredEntry(id, payload), payload),
        false,
        "the propagated grave refuses the resurrection",
      );
    ...
    test("expiry writes NO grave (lazy read path AND sweeper); clear writes one per live entry; drop of an absent id STILL writes a grave (propagation)", async () => {
      ...
      assert.deepEqual(
        await s1.tombstones(),
        [],
        "an expired entry leaves no grave -- its terms travel with it",
      );
      ...
      assert.equal(
        g3.length,
        1,
        "but the id is tombstoned -- a grave propagates through a node that never held it",
      );
    ...
    test("concurrent same-id destroyers converge to ONE grave with a clean verdict -- no raw fs error escapes", async () => {
      ...
      const popDrain = async (s, id) => {
        try {
          await drain(await s.pop(id));
        } catch (e) {
          if (!(e instanceof RefClaimed) && !(e instanceof RefNotFound)) throw e;
        }
      };
      const scenarios = [
      ...
      for (const racers of scenarios) {
        ...
        for (const r of settled) {
          if (r.status === "rejected") {
            assert.ok(
              r.reason instanceof StashError,
              "a losing racer rejects TYPED, never a raw fs error: " + (r.reason && r.reason.stack),
            );
          }
        }
        ...
        assert.equal(graves[0].id, id);
        assert.ok(
          ["pop", "drop", "clear", "spent"].includes(graves[0].cause),
          "the grave records a valid cause",
        );
      }
    });
    ...
    test("a budgeted entry replicated to both stores is eventually-once: each serves one drain, both bury it 'spent'", async () => {
      ...
      assert.deepEqual(
        await drain(await B.apply(id)),
        payload,
        "B serves its one read too (eventually-once)",
      );
      for (const s of [A, B]) {
    ...
    test("tombstones survive prune() until tombstoneTtl, then are pruned; after pruning store() of that id succeeds (the floor-rule consequence)", async () => {
      ...
      await pollUntil(
        async () => {
          await stash.prune();
          return (await stash.tombstones()).length === 0;
        },
        { timeout: 3000 },
      );
      assert.equal(
        await stash.store(makeStoredEntry(id, "reborn"), "reborn"),
        true,
        "past the ttl, the id can be stored again -- the documented floor rule",
      );
    });
    ...
    test("tombstoneTtl: null never prunes -- a grave survives arbitrarily many prune() calls", async () => {
      ...
      assert.equal(
        (await stash.tombstones()).length,
        1,
        "null tombstoneTtl keeps the grave forever",
      );
    });
    ...
    test("the backend tombstone contract: first-write-wins, and absent/traversal answers on the subpath surface", async () => {
      ...
      assert.deepEqual(
        await backend.listTombstones().then((l) => l.filter((t) => t.id !== id)),
        [],
      );
      assert.equal(
        await backend.removeTombstone(generate()),
        false,
        "removing an absent grave is false",
      );
      ...
      for (const bad of ["../escape", "v1_..", "not-a-ref"]) {
        ...
        await assert.rejects(
          backend.writeTombstone(bad, { id: bad, destroyedAt: 1, cause: "drop" }),
          InvalidRef,
        );
...
suite("M3 lifecycle: sweeper, close, disposal (SPEC.md 7, 7.1)", () => {
  ...
  test("close() awaits an in-flight sweep: no deletion lands after it resolves", async () => {
    ...
    const backend = {
      ...
      stats: (...a) => inner.stats(...a),
      list: async () => {
        sweepEnteredList = true;
        await listGate;
        return inner.list();
      },
      remove: async (id) => {
        if (closeResolved) removedAfterClose = true;
        return inner.remove(id);
      },
    };
    const stash = new Stash({ backend, sweepInterval: 5 });
    ...
    const closePromise = stash.close().then(() => {
      closeResolved = true;
    });
    ...
    assert.equal(
      closeResolved,
      false,
      "close() awaited the in-flight sweep instead of resolving early",
    );
    ...
    assert.equal(
      removedAfterClose,
      false,
      "the sweep's deletion happened before close() resolved, not after",
    );
  });

  test("a sweepInterval above Node's timer ceiling, or of zero, is a config error", async () => {
    assert.throws(
      () => new Stash({ backend: new MemoryBackend(), sweepInterval: "30d" }),
      TypeError,
    );
    assert.throws(() => new Stash({ backend: new MemoryBackend(), sweepInterval: 0 }), TypeError);
    assert.throws(
      () => new Stash({ backend: new MemoryBackend(), sweepInterval: C.TIME.MAX_TIMER_MS + 1 }),
      TypeError,
    );
  ...
  test("a default ttl whose expiresAt would overflow is refused at construction, not at first push", () => {
    ...
    assert.throws(
      () => new Stash({ backend: new MemoryBackend(), ttl: Number.MAX_SAFE_INTEGER }),
      TypeError,
    );
    ...
    return ok
      .push("x")
      .then((ref) => ok.show(ref))
      .then((e) => assert.equal(e.expiresAt, e.createdAt + 86400000));
  });

  test(
    "a process with an open sweeping Stash exits on its own (the unref rule)",
    { skip: SANDBOXED, timeout: 15000 },
    async () => {
      ...
      const rv = spawnSync(process.execPath, ["--input-type=module", "-e", code], {
        timeout: 10000,
        encoding: "utf8",
      });
      assert.equal(
        rv.signal,
        null,
        "child was not killed -- the unref'd timer let it exit: " + (rv.stderr || ""),
      );
...
for (const { name, create } of BACKENDS) {
  suite("backend contract: " + name, () => {
    ...
    test("stats() is { entries, bytes, claimed } of non-negative safe integers, and tracks writes and removes", async () => {
      ...
      for (const v of [s.entries, s.bytes, s.claimed])
        assert.equal(Number.isSafeInteger(v) && v >= 0, true);
  ...
  suite("M9 digest agility: " + name, () => {
    for (const algo of DIGEST_ALGOS) {
      test(`push+apply round-trips under digest '${algo}'; the stored digest is the algo's hash, self-describing, verified on read`, async () => {
        ...
        assert.equal(
          stored.digest,
          digestOf(payload, algo),
          "the stored digest is the algorithm's hash of the bytes",
        );
        assert.ok(stored.digest.startsWith(algo + ":"), "the stored digest names its algorithm");
        assert.deepEqual(
          await drain(await stash.apply(ref)),
          payload,
          "the read verifies with the stored algorithm",
        );
      });
    }
    ...
    test("a store holds MIXED algorithms: a sha256 and a sha3-512 entry each apply and verify() clean (self-describing per entry)", async () => {
      ...
      assert.deepEqual(
        await drain(await s256.apply(b)),
        Buffer.from("second, sha3-512"),
        "a sha256-configured Stash still verifies a sha3-512 entry",
      );
      assert.deepEqual(await drain(await s3.apply(a)), Buffer.from("first, sha256"));
      assert.deepEqual(
        (await s256.verify()).findings,
        [],
        "the mixed-algorithm store verifies clean",
      );
    });
    ...
    test("the digest option is a closed enum: an unknown or non-string algorithm is a config-time TypeError; every registry algorithm constructs", () => {
      ...
      for (const algo of DIGEST_ALGOS)
        assert.doesNotThrow(() => new Stash({ backend: create(), digest: algo }));
    });
    ...
    test("a backend built to the DOCUMENTED write(id, source, entry) signature still honors the digest selection -- it rides in the entry, not an extra argument", async () => {
      ...
      const documented = wrapBackend(inner, {
        write: (id, source, entry) => inner.write(id, source, entry),
      });
      ...
      assert.ok(
        (await stash.show(ref)).digest.startsWith("sha3-512:"),
        "the sha3-512 selection survived a three-argument backend",
      );
      assert.deepEqual(
        await drain(await stash.apply(ref)),
        Buffer.from("bytes under a strictly-three-arg backend"),
        "and the entry still verifies",
      );
    });
  });
}
...
suite("config-time failures", () => {
  ...
  test("maxSize above maxTotal is a config-time TypeError (a per-entry cap that can never bind)", () => {
    ...
    assert.throws(
      () => new Stash({ backend: new MemoryBackend(), maxSize: 100, maxTotal: 50 }),
      TypeError,
    );
    assert.throws(
      () => new Stash({ backend: new MemoryBackend(), maxSize: "2gb", maxTotal: "1gb" }),
      TypeError,
    );
