...
async function applyDrain(stash, ref) {
  ...
  await pollUntil(async () => {
    try {
      out = await drain(await stash.apply(ref));
      return true;
    } catch (e) {
      if (e && e.code === "ECLAIMED") return false;
      throw e;
    }
  });
  return out;
...
for (const { name, create } of BACKENDS) {
  suite("chaos: " + name, () => {
    ...
    test("apply fails closed when the backend read throws -- the entry survives", async () => {
      ...
      const backend = wrapBackend(inner, {
        read: (...a) => {
          if (faultOn) throw new Error("read fault");
          return inner.read(...a);
        },
      });
      const stash = new Stash({ backend, sweepInterval: null });
    ...
    test("pop fails closed when the backend claim throws -- the entry is not destroyed", async () => {
      ...
      const backend = wrapBackend(inner, {
        claim: (...a) => {
          if (faultOn) throw new Error("claim fault");
    ...
    test("store fails closed when the backend write throws -- nothing lands", async () => {
      ...
      const backend = wrapBackend(inner, {
        write: (...a) => {
          if (faultOn) throw new Error("write fault");
      ...
      await assert.rejects(
        stash.store(makeStoredEntry(freshId, bytes, { digest: digestOf(bytes) }), bytes),
    ...
    test("a push whose source dies mid-stream leaves nothing on the store (SPEC.md 8)", async () => {
      ...
      await assert.rejects(
        stash.push(erroringSource(16 * C.BYTES.KIB, 3)),
        (e) => e instanceof Error,
      );
    ...
    test(
      "a budgeted apply aborted mid-drain spends NO read (SPEC.md 4.1)",
      { timeout: 10000 },
      async () => {
        const stash = new Stash({ backend: create(), sweepInterval: null });
        ...
        await pollUntil(async () => {
          try {
            return (await stash.show(ref)).readsLeft === 2;
          } catch {
            return false;
          }
        });
    ...
    test(
      "a pop aborted mid-drain restores under the default policy; the entry survives",
      { timeout: 10000 },
      async () => {
        ...
        assert.deepEqual(
          await applyDrain(stash, ref),
          big,
          "a torn pop restored the entry, no bytes lost",
        );
        await stash.close();
      },
    );
    ...
    test(
      "a pop aborted mid-drain under 'burn' destroys the entry",
      { timeout: 10000 },
      async () => {
        ...
        assert.ok(
          (await stash.tombstones()).some((g) => g.id === ref),
          "a burned entry leaves a grave -- no resurrection",
        );
        await stash.close();
      },
    );
    ...
    test(
      "recovery never reclaims a claim a live reader is mid-drain on; an orphan still is (SPEC.md 6)",
      { timeout: 10000 },
      async () => {
        const inner = create();
        ...
        await pollUntil(async () => {
          await stash.list();
          return !(await inner.isClaimed(orphanRef));
        });
        assert.equal(
          ended,
          false,
          "the live reader has not completed -- its claim is genuinely mid-drain",
        );
        assert.equal(
          await inner.isClaimed(liveRef),
          true,
          "the live reader's claim survived the recovery re-scan",
        );
        assert.equal(
          (await stash.tombstones()).some((g) => g.id === liveRef),
          false,
        ...
        assert.deepEqual(
          await drain(stream),
          big,
          "the mid-drain read completes with the full, correct bytes",
        );
        await pollUntil(async () => !(await present(stash, liveRef)));
        assert.ok(
          (await stash.tombstones()).some((g) => g.id === liveRef),
          "the reader's own pop destroyed it exactly once",
        );
        await stash.close();
      },
    );
    ...
    test(
      "recovery RESTORES a crash orphan even under 'burn' -- it never burns a crashed read (SPEC.md 6)",
      { timeout: 10000 },
      async () => {
        ...
        await pollUntil(async () => {
          await stash.list();
          return !(await inner.isClaimed(ref));
        }); // recovery resolves the orphan
        assert.equal(await stash.has(ref), true, "the crash orphan is restored, not burned");
        assert.equal(
          (await stash.tombstones()).some((g) => g.id === ref),
          false,
          "no grave -- recovery destroyed nothing",
        );
        assert.deepEqual(
          await applyDrain(stash, ref),
          Buffer.from("crash-orphan bytes"),
          "the restored entry reads back intact",
        );
        await stash.close();
      },
    );
    ...
    test(
      "a FAULTED expired-restore in the claim window releases the live-claim guard, so recovery still reclaims the orphan (SPEC.md 6)",
      { timeout: 10000 },
      async () => {
        ...
        const backend = wrapBackend(inner, {
          ...
          claim: async (...a) => {
            const r = await inner.claim(...a);
            return { ...r, entry: { ...r.entry, expiresAt: 1 } };
          },
          restore: (...a) => {
            if (failRestore) throw new Error("restore boom");
        ...
        await pollUntil(
          async () => {
            await stash.prune();
            return !(await inner.isClaimed(leakRef));
          },
          { timeout: 6000 },
        );
        assert.equal(
          await inner.isClaimed(leakRef),
          false,
          "the faulted expired-restore did not pin leakRef in the live-claim guard",
        );
        await stash.close();
      },
    );
    ...
    test(
      "a faulted claim resolution schedules its OWN recovery re-scan, so an orphan with no concurrent claim is still reclaimed (SPEC.md 6)",
      { timeout: 10000 },
      async () => {
        ...
        assert.equal(
          await inner.isClaimed(leakRef),
          false,
          "orphan from a faulted restore with no concurrent claim is still reclaimed",
        );
        await stash.close();
      },
    );
    ...
    test(
      "a faulted resolution's forced re-scan is not erased by an in-flight recovery scan writing back a stale deadline (SPEC.md 6)",
      { timeout: 15000 },
      async () => {
        ...
        const backend = wrapBackend(inner, {
          ...
          listClaims: async () => {
            ...
            return [
              ...(v ? [v] : []),
              ...claims.filter((c) => c.id !== victimRef && c.id !== orphanRef),
              ...(o ? [o] : []),
            ];
          },
          restore: async (id) => {
            if (id === orphanRef && parkInRestore) {
              parkInRestore = false;
              scanParked = true;
              await scanGate;
            } // park the in-flight scan mid-resolve
            return inner.restore(id);
          },
          commit: async (id) => {
            if (id === victimRef && failVictimCommit) {
              failVictimCommit = false;
        ...
        await pollUntil(
          async () => {
            await stash.prune();
            return !(await inner.isClaimed(victimRef));
          },
          { timeout: 200 },
        );
        assert.equal(
          await inner.isClaimed(victimRef),
          false,
          "the in-flight scan did not erase the faulted resolution's forced re-scan",
        );
        await stash.close();
      },
    );
    ...
    test(
      "two Stash over one store share the live-claim guard: one instance's recovery never reclaims the other's mid-drain read (SPEC.md 6)",
      { timeout: 10000 },
      async () => {
        ...
        assert.equal(
          await backend.isClaimed(ref),
          true,
          "B's recovery did not reclaim A's live mid-drain claim",
        );
        ...
        assert.deepEqual(
          await drain(stream),
          big,
          "A's mid-drain read completes with the full bytes",
        );
    ...
    test(
      "closing a Stash while its pop stream is still draining keeps the store's guard, so a later Stash's recovery does not reclaim the live claim (SPEC.md 6)",
      { timeout: 10000 },
      async () => {
        ...
        assert.equal(
          await backend.isClaimed(ref),
          true,
          "the live claim survived a later Stash's recovery after close()",
        );
    ...
    test(
      "two Stash over one identity-less backend object share the live-claim guard (SPEC.md 6)",
      { timeout: 10000 },
      async () => {
        ...
        await b.prune();
        assert.equal(
          await shared.isClaimed(ref),
          true,
          "B's recovery skipped A's live claim over the shared identity-less backend",
        );
    ...
    test(
      "a claim age-reclaimed DURING acquisition (before its guard is recorded) cannot resurrect a once-only entry for a second reader (SPEC.md 6)",
      { timeout: 10000 },
      async () => {
        ...
        const backend = wrapBackend(inner, {
          ...
          listClaims: async () => {
            ...
            return claims.map((c) =>
              c.id === victimRef ? { ...c, claimedAt: c.claimedAt - 10 * 60_000 } : c,
            );
          },
        });
        ...
        await assert.rejects(
          stash.apply(victimRef),
          "a second reader must not acquire a once-only entry another read holds",
        );
    ...
    test(
      "a live claim SKIPPED by a recovery scan keeps recovery scheduled, so a later faulted resolution leaving an orphan is still reclaimed (SPEC.md 6)",
      { timeout: 10000 },
      async () => {
        ...
        const backend = wrapBackend(inner, {
          commit: async (id) => {
            if (id === victimRef && failCommitOnce) {
        ...
        assert.equal(
          await inner.isClaimed(victimRef),
          false,
          "a skipped live claim that later orphans via a faulted commit is still reclaimed",
        );
        await stash.close();
      },
    );
    ...
    test("recovery resolves a corrupt-sidecar orphan claim instead of wedging every verb on EINTEGRITY (SPEC.md 6)", async () => {
      ...
      const backend = wrapBackend(inner, {
        ...
        listClaims: async () =>
          (await inner.listClaims()).map((c) =>
            c.id === orphanRef ? { ...c, claimedAt: c.claimedAt - 10 * 60_000 } : c,
          ),
        // ... and make its sidecar unreadable, the crash-mid-rewrite corruption.
        stat: async (id) => {
          if (id === orphanRef) throw new IntegrityError("sidecar storage shape is damaged");
          return inner.stat(id);
        },
        commit: async (id) => {
          if (id === orphanRef) commits += 1;
          return inner.commit(id);
        },
      });
      ...
      assert.deepEqual(
        await stash.list(),
        [],
        "recovery reaped the corrupt orphan; the store is not wedged",
      );
      assert.equal(
        commits,
        1,
        "recovery finished the corrupt entry's destruction via commit, never a rethrow",
      );
      assert.equal(
        (await stash.tombstones()).length,
        0,
        "the corrupt-entry cleanup writes no grave",
      );
      // And every verb stays live -- no EINTEGRITY poisoning the memoized recover.
      const fresh = await stash.push("healthy");
      assert.ok(
        (await stash.list()).some((e) => e.id === fresh),
        "the store still accepts and lists new writes",
      );
      await stash.close();
    });
  });
...
suite("chaos: guard reclamation on GC", () => {
  test(
    "a Stash GC'd while holding a leaked pop clears its stale guard, so a later Stash reclaims the claim (SPEC.md 6)",
    { skip: typeof global.gc !== "function", timeout: 15000 },
    async () => {
      const CLAIM_TIMEOUT = 40;
      ...
      await pollUntil(
        async () => {
          await b.prune();
          return !(await backend.isClaimed(ref));
        },
        { timeout: 6000 },
      );
      assert.equal(
        await backend.isClaimed(ref),
        false,
        "the abandoned claim from a collected Stash is reclaimable, not pinned",
      );
      await b.close();
    },
  );
